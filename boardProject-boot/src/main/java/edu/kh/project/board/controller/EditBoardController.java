...
public class EditBoardController {
	...
	public String boardUpdate(@PathVariable("boardCode") int boardCode,
		...
		if(board == null) {
			...
			// 해당 글 상세 조회로 리다이렉트. (/board/1/2001)
			...
			path = "board/boardUpdate"; // templates/board/boardUpdate.html forward.
	 ...
	 * @param loginMember	: 로그인 한 회원의 번호를 이용. ( 로그인 == 작성자 )
	...
	public String boardUpdate(@PathVariable("boardCode") int boardCode,
		...
		if(result > 0) {
			...
			message = "수정 실패";
			path = "update"; // GET (수정 화면 전환) 리다이렉트 하는 상대 경로.
		}
	 ...
	 * @param boardNo		: 게시글 번호.
	 * @param cp			: 삭제 시 게시글 목록으로 리다이렉트할 때 사용할 페이지 번호.
	 * @param loginMember	: 현재 로그인 한 회원 번호 사용.
	 * @param ra			: 리다이렉트 시, request scope 로 값 전달용.
	 * @return
	 */
