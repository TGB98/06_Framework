...
@Slf4j
@Component // Bean 등록.(아무 역할은 아니지만 Bean으로 등록하기 위함.)
...
//	@Scheduled(cron = "0 0 0 1 * *") 매달 1일.
	...
	public void scheduling() {
		...
		// 두 배열 하나로 합치기.
		// imageArr 라는 빈 배열을 boardArr와 memberArr 길이 만큼의 크기로 만듦.
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사. (깊은 복사.)
		// System.arraycopy(복사할 배열, 몇번인덱스부터 복사할지, 새로운 배열, 새로운 배열의 몇번부터 넣을지 인덱스번호, 복사를 어디까지 할건지)
		...
		if(!serverImageList.isEmpty()) { // 서버에 이미지가 있을 경우.
			...
			for(File serverImage : serverImageList) {
				...
				// List.indexOf(객체) :
				// List에 객체가 존재하면 존재하는 index 를 반환.
				// 존재하지 않는다면 -1 을 반환하는 메서드.
