...
public class EmailServiceImpl implements EmailService{
	...
	// SpringTemplateEngine : 타임리프(템플릿 엔진)를 이용해서 html 코드 -> java 코드 변환.
	...
	public String sendEmail(String htmlName, String email) {
		...
		log.debug("map : " + map); // map : {authKey=cd13af, email=user02@kh.or.kr}
...
//			helper.setText("인증번호입니다 : " + authKey);
	...
	// HTML 템플릿에 데이터를 바인딩해 최종 HTML 생성.
	...
	@Transactional(rollbackFor=Exception.class) // 메서드 레벨에서도 이용 가능함.( 해당 메서드에서만 트랜잭션 커밋/롤백 )
	private boolean storeAuthKey(Map<String, String> map) {
		...
		// 2. update 실패 (== 기존 데이터 없음) insert 수행.
		if(result == 0) {
	...
	// 매우 낮은 확률로 중복되는 식별자를 생성함.
	// 주로 데이터베이스 기본 키, 고유한 식별자를 생성해야 할 때 사용한다.
