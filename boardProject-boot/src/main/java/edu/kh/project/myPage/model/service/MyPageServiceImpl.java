...
public class MyPageServiceImpl implements MyPageService{
	...
	public int changePw(int memberNo, Map<String, Object> paramMap) {
		...
		// 2. 같을 경우 새 비밀번호 암호화 (BCryptPasswordEncoder.encode(평문))
	...
	public int secession(String memberPw, int memberNo) {
		...
		// 현재 로그인 한 회원의 암호화된 비밀번호를 DB에서 조회 해오기.
		String originPw = mapper.selectPw(memberNo);
	...
	public String fileUpload1(MultipartFile uploadFile) throws Exception{
		...
		//   원하는 경로에 실제로 전송.(서버의 어떤 폴더에 저장할지 지정.)
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		 ...
		 * DB 부하를 줄이기 위해서
		 * 
		 * 1) DB 에는 서버에 저장할 파일 경로를 저장한다.
		 * 
		 * 2) DB 삽입/수정 성공 후 서버에 파일을 저장.
		 * 
		 * 3) 만약에 파일 저장 실패 시
		 * -> 예외 발생
		 * -> @Transactinal 를 이용해 rollback 수행.
		 * 
		 * */
		...
		// 2. DB에 전달할 데이터를 'DTO'로 묶어서 INSERT 호출 하기.
...
//		log.debug("fileRename : " + fileRename);
		...
		// 장점 2. method chaining 을 이용해 한 줄로 작성 가능함.
		...
		// 삽입 성공 시
		// C:/uploadFiles/test/변경된파일명 으로 파일을 서버 컴퓨터에 저장.
