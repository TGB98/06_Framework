...
// 기존코드 (myPage-info.html 외에는 문제가 발생할 수 있는 코드이다.)
...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async (e) => {
        ...
        // 기존 닉네임과 새로 입력된 닉네임이 다르면 중복검사 시도.
        // -> 같으면 변경된 적 없다 -> 중복검사 진행X.
        
        if(currentNickname !== memberNickname.value) {

            // 비동기 요청 (fetch() API 이용)
            // async / await 사용
            // async : 비동기 함수를 만들 때 사용하는 키워드. ("이 함수 내에는 오래 걸리는 작업이 있다라는 뜻.")
            // await : 비동기 작업의 결과를 기다릴 때 사용. -> 반드시 async 함수 안에서만 사용 가능함.
            //          -> "이 작업이 끝날때까지 기다려" 달라는 뜻.
...
let previousImage = profileImg.src; // 이전 이미지 URL 기록. (초기 상태의 이미지 URL을 저장.)
let previousFile = null; // 이전 선택된 파일 객체를 저장.
...
imageInput.addEventListener("change", () => {
    ...
    if(file) { // 파일이 선택된 경우.
        if(file.size <= MAX_SIZE) { // 파일 크기가 허용 범위 이내인 경우.
            const newImageUrl = URL.createObjectURL(file); // 임시 URL 생성함.
            // 미리보기 이미지 URL 용도.
            profileImg.src = newImageUrl; // 미리보기 이미지 설정.(img 태그의 src에 선택한 파일의 임시 경로를 대입.)
            statusCheck = 1; // 새 이미지 선택한 상태를 기록.
            previousImage = newImageUrl; // 현재 선택된 이미지를 이전 이미지로 저장. (다음에 바뀔 때를 대비.)
            previousFile = file; // 현재 선택된 파일 객체를 이전 파일로 저장 (다음에 바뀔 때를 대비.)
            ...
            if(previousFile) { 
                ...
                // -> <input type="file"> 요소의 파일을 동적으로 설정 가능하다.
                // --> input 태그의 files 속성은 FileList 만 저장 가능하기 때문에
                // DataTransfer를 이용하여 현재 File 객체를 FileList 로 변환하여 할당.
